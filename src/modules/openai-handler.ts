...
class OpenAIAction {
	...
	async suggestGame(level: number, genre: string, model: string) {
		const command = this.generateCommand(
			...
			'お題のゲームを1つ提示してください。一流が作るとしたら1日で作れるレベルのものを提示してください。また、提示ゲームの満たすべき（最低限の）仕様を最低3つ箇条書きで書いてください。レベル5以上の場合は、この世の中に存在しない非常に難しいものを提示してください。',
			'お題のレベルとジャンルを指定する。',
			'お題のゲームを1つと、満たすべき仕様を4つ箇条書きで提示。マークダウンで回答する。回答は6行以内とする。'
		);
